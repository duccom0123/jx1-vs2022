...
void CRootClient::OnServerEventCreate()
{
	...
	KGLogPrintf(LOG_INFO,"root client create: %s\n", _ip2a(GetSvrIP()));
...
void CRootClient::OnServerEventClose()
{
	...
	KGLogPrintf(LOG_INFO,"root client close: %s\n", _ip2a(GetSvrIP()));
...
BOOL CRootClient::StartAutoTime()
{
	if (!m_isStartAutoTime)
	{
		...
		KGLogPrintf(LOG_INFO,"start autotimer for Ping: %s\n", _ip2a(GetSvrIP()));
	}
...
BOOL CRootClient::StopAutoTime()
{
	if (m_isStartAutoTime)
	{
		...
		KGLogPrintf(LOG_INFO,"stop autotimer for Ping: %s\n", _ip2a(GetSvrIP()));
	}
...
void CRootClient::AutoTime()
{
	...
	if (tick - m_LastSendPingTime >= 120000)	//两分钟没有发出Ping
	{
		...
		*pPingTick = m_LastSendPingTime;
		...
		KGLogPrintf(LOG_INFO,"root client ping: %s\n", _ip2a(GetSvrIP()));
	}
}
	...
	else if (pHeader->ProtocolID == s2c_gatewayinfo)
	{
		...
		if (size == sizeof(KServerInfo) + 2)
		{
			if (pServer->nServerType == server_Login)
			{
				...
				if (!sockdup.IsValid())
				{
					KGLogPrintf(LOG_INFO,"warning: relay client exist: %s\n", addr.c_str());
					return;
				}

				KGLogPrintf(LOG_INFO,"relay client attempt: %s\n", addr.c_str());
				if (!gConnectToSibling(addr.c_str()))
				{
					KGLogPrintf(LOG_INFO,"error: relay client error: %s\n", addr.c_str());
					return;
				}
