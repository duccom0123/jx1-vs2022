...
BOOL CChannelMgr::Initialize()
{
	...
	if (!nameChannGM.empty())
	{
		...
		KGLogPrintf(LOG_INFO,"Create Channel: [Stock, GM] <%08X> %s\n", m_channidGM, nameChannGM.c_str());
	}
	...
	for (std::vector<std::_tstring>::const_iterator it = rVecChann.begin(); it != rVecChann.end(); it++)
	{
		...
		KGLogPrintf(LOG_INFO,"Create Channel: [Stock] <%08X> %s\n", channid, channname.c_str());
	}
	...
	else
	{
		if (force)
		{
			...
			if (channid != DWORD(-1))
			{
				...
				KGLogPrintf(LOG_INFO,"GM Create Channel: <%08X> %s\n", channid, channel.c_str());
...
BOOL CChannelMgr::GmFreeChannID(DWORD channid)
{
	...
	{
		KGLogPrintf(LOG_INFO,"GM Destroy Channel: <%08X> %s\n", channid, rChannInfo.channname.c_str());
...
//	KGLogPrintf(LOG_INFO,"Chat Channel: sentence %u Bs from [%s] on <%08X>", pChannelChatCmd->sentlen, srcrole.c_str(), pChannelChatCmd->channelid);
...
BOOL CChannelMgr::Block_QueryChannelID(const std::_tstring& channel, DWORD ip, unsigned long param, DWORD nameid)
{
	...
	KGLogPrintf(LOG_INFO,"Query ChannelID: %s from (%08X, %08X)\n", channel.c_str(), ip, param);
	...
	else
	{
		...
		if (channid != DWORD(-1))
		{
			...
			KGLogPrintf(LOG_INFO,"Create Channel: <%08X> %s\n", channid, channname.c_str());
...
BOOL CChannelMgr::Block_Subscribe(DWORD ip, unsigned long param, DWORD channelid)
{
	...
	{
		KGLogPrintf(LOG_INFO,"Subscribe Channel: GM <%08X> by (%08X,%08X)\n", channelid, ip, param);
		return TRUE;
	}
	...
	KGLogPrintf(LOG_INFO,"Subscribe Channel: %08X by (%08X,%08X)\n", channelid, ip, param);
...
BOOL CChannelMgr::Block_Unsubscribe(DWORD ip, unsigned long param, DWORD channelid)
{
	...
	KGLogPrintf(LOG_INFO,"Unsubscribe Channel: %08X by (%08X,%08X)\n", channelid, ip, param);
...
BOOL CChannelMgr::Block_FreeChannID(DWORD channid, DWORD ip, unsigned long param)
{
	...
	KGLogPrintf(LOG_INFO,"Free ChannelID: <%08X> from (%08X, %08X)\n", channid, ip, param);
