// -------------------------------------------------------------------------
//	ÎÄ¼₫Ăû		£º	UiReconnect.h
//	´´½¨Ơß		£º	Freeway Chen
//	´´½¨Ê±¼ä	£º	2003-7-18
//	¹¦ÄÜĂèÊö	£º	×Ô¶¯ÖØÁ¬µÄÁ÷³̀¿ØÖÆ	
// -------------------------------------------------------------------------
#include "KWin32.h"
#include "KIniFile.h"
...
#define	MSG_RECONNECTING				"HÖ thèng ®ang kƠt nèi l¹i lÇn thø <color=255,0,0>%d<color>\n%s"
#define	MSG_WAIT_TO_RECONNECT			"TiƠn hµnh kƠt nèi l¹i trong %d gi©y (lÇn thø <color=255,0,0>%d<color>)"
#define	MSG_WAIT_NEXT_GROUP_RECONNECT	"TiƠn hµnh kƠt nèi l¹i trong %d gi©y"
#define	MSG_HIDE_RECONNECT_MSG_ID		"21"

//Á½×é¶ÏÏßÖØÁ¬µÄ¼ä¸ôÊ±¼ä£¬µ¥Î»ºÁĂë
#define RECONNECT_GROUP_INTERVAL		60000
//Á½´Î¶ÏÏßÖØÁ¬µÄ¼ä¸ôÊ±¼ä£¬µ¥Î»ºÁĂë
#define RECONNECT_INTERVAL				10000
//¿ªÊ¼Ê±²»¿É¼ûµÄÖØÁ¬µÄ´ÎÊư
#define INVISIBLE_RECONNECT_TIMES		3
//²»¿É¼ûµÄÁ½´Î¶ÏÏßÖØÁ¬µÄ¼ä¸ôÊ±¼ä£¬µ¥Î»ºÁĂë
#define INVISIBLE_RECONNECT_INTERVAL	5000

//È¡ÏûÖØÁ¬µÄ°´Å¥µÄÎÄ×Ö
#define	RECONNECT_QUIT_BTN_LABEL	"Tho¸t"
//Á¢¼´ÖØÁ¬µÄ°´Å¥µÄÎÄ×Ö
#define	RECONNECT_START_AT_ONCE		"KƠt nèi l¹i"
	...
	case LL_S_IDLE:	//====ÖØÁ¬Ê§°Ü====
		{
			...
			{
				m_bStop = true;
				pReconnectMsg = "Kh«ng ®ñ thêi gian ®Ó ch¬i!. H·y n¹p card ®Ó tiƠp tôc trß ch¬i!";
			}
			else if (eResult == LL_R_ACCOUNT_FREEZE)
			{
				m_bStop = true;
				pReconnectMsg = "Tµi kho¶n nµy ®· b̃ khăa";
			}
			else
			{
				m_bWaitToReconnect = true;
				...
				else
				{
					...
					pReconnectMsg = "KƠt nèi thÊt b¹i";
				}
			}
		}
		break;
	case LL_S_ACCOUNT_CONFIRMING:	//µÈ´ưƠËºÅĂÜÂëÑéÖ¤
		pReconnectMsg = "§ang tiƠn hµnh kiÓm tra tµi kho¶n vµ mËt khÈu";
		break;
	case LL_S_WAIT_ROLE_LIST:		//µÈ´ư½ÓÊƠ½ÇÉ«ÁĐ±íÊư¾Ư
		pReconnectMsg = "§ang nhËn danh s¸ch nh©n vËt";
		break;
	case LL_S_WAIT_TO_LOGIN_GAMESERVER:	//µÈ´ưµÇÂ½ÓÎÏ··₫ÎñÆ÷
		pReconnectMsg = "§ang phôc vô trß ch¬i";
		break;
	case LL_S_ENTERING_GAME:		//ƠưÔÚ½øÈëÓÎÏ·
...
int KReconnectWnd::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)//´°¿Úº¯Êư
{
