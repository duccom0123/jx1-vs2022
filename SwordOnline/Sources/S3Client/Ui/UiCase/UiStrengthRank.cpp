			...
			if(m_pCurrentIndexNode->bValueAppened)
			{
			    ...
			    if(m_pCurrentIndexNode->szUnit[0])
			    {
					if (m_pCurrentIndexNode->usIndexId == enumTopTenMasterHand || 
						...
						sprintf(szBuf, "C�p %d", pNode->nValueAppend % MAX_LEVEL + (pNode->nValueAppend % MAX_LEVEL == 0 ? MAX_LEVEL : 0), pNode->nValueAppend / MAX_LEVEL - (pNode->nValueAppend % MAX_LEVEL == 0 ? 1 : 0));
						...
						)
					{
						if (pNode->nValueAppend > MONEY_FLOOR)
							sprintf(szBuf, "%d v�n l��ng", pNode->nValueAppend / MONEY_FLOOR);
						else
