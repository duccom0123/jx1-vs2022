...
void g_Pal24ToPal16Style(KPAL24 *pPal24, KPAL16 *pPal16, int nColors)
{
    while (nColors-- > 0)
    {
        *pPal16 = g_RGB(pPal24->Red * m_ColorStyle.Red / 255, pPal24->Green * m_ColorStyle.Green / 255, pPal24->Blue * m_ColorStyle.Blue / 255);
        pPal16++;
        pPal24++;
    }
}
...
void g_Pal32ToPal16Style(KPAL32 *pPal32, KPAL16 *pPal16, int nColors)
{
    while (nColors-- > 0)
    {
        *pPal16 = g_RGB(pPal32->Red * m_ColorStyle.Red / 255, pPal32->Green * m_ColorStyle.Green / 255, pPal32->Blue * m_ColorStyle.Blue / 255);
        pPal16++;
        pPal32++;
    }
}
...
void g_Pal24ToPal16Lum(KPAL24 *pPal24, KPAL16 *pPal16, int nColors)
{
    ...
    for (lum = 32; lum > 0; lum--)
    {
        for (i = 0; i < nColors; i++)
        {
            pPal16[i] = g_RGB(pPal24[i].Red * lum / 32, pPal24[i].Green * lum / 32, pPal24[i].Blue * lum / 32);
        }
...
void g_Pal32ToPal16Lum(KPAL32 *pPal32, KPAL16 *pPal16, int nColors)
{
    ...
    for (lum = 32; lum > 0; lum--)
    {
        for (i = 0; i < nColors; i++)
        {
            pPal16[i] = g_RGB(pPal32[i].Red * lum / 32, pPal32[i].Green * lum / 32, pPal32[i].Blue * lum / 32);
        }
        pPal16 += nColors * sizeof(KPAL16);
    }
}
