...
class CTextFilter : public ITextFilter
{
  ...
  private:
    ...
    enum
    {
        HASHSIZE = 0x01 << (sizeof(_TUCHAR) * 8)
    };
    typedef _TUCHAR HASHINDEXTYPE;

    HASHINDEXTYPE Char2HashIdx(TCHAR ch)
    {
        return HASHINDEXTYPE(ch);
    }
  ...
  public:
    static CTextFilter *CreateInstance()
    {
        return new CTextFilter;
    }
};
...
BOOL CTextFilter::IsTextPass(LPCTSTR text)
{
    try
    {
        ...
        {
            if ((loop == 0 && bAdvPrior) || (loop != 0 && !bAdvPrior))
            {   //advance
...
ENGINE_API ITextFilter *CreateTextFilter()
{
