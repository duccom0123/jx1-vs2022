...
{
    if (g_pRepresent == NULL || m_pPrerenderGroundImg == NULL || (bForce == false && m_pPrerenderGroundImg->GROUND_IMG_OK_FLAG))
        return true;

    m_pPrerenderGroundImg->GROUND_IMG_OK_FLAG = true;

    g_pRepresent->ClearImageData(m_pPrerenderGroundImg->szImage, m_pPrerenderGroundImg->uImage, m_pPrerenderGroundImg->nISPosition);
    ...
    for (nIndex = 0; nIndex < m_GroundLayerData.uNumGrunode; nIndex++)
    {
        ...
        {
            g_pRepresent->DrawPrimitivesOnImage(LOCAL_MAX_IMG_NUM, &ImgList[0], RU_T_IMAGE, m_pPrerenderGroundImg->szImage, m_pPrerenderGroundImg->uImage, m_pPrerenderGroundImg->nISPosition);
            nNum = 0;
            pGi = &ImgList[0];
        }
    }
    ...
    if (nNum)
    {
        g_pRepresent->DrawPrimitivesOnImage(nNum, &ImgList[0], RU_T_IMAGE, m_pPrerenderGroundImg->szImage, m_pPrerenderGroundImg->uImage, m_pPrerenderGroundImg->nISPosition);
    }
    return true;
}
...
void KScenePlaceRegionC::SetNestRegion(KScenePlaceRegionC *pNest)
{
    ...
    {
        if (pObj->bRelateRegion == OBJ_IS_SELF_OWNED && pObj->nPositionX < rcthis.right && pObj->nPositionX + pObj->nWidth > rcthis.left && pObj->nPositionY < rcthis.bottom && pObj->nPositionY + pObj->nHeight + pObj->nHeight > rcthis.top)
        {
    ...
    pObj = (KSPRCoverGroundObj *)realloc(m_GroundLayerData.pObjects, sizeof(KSPRCoverGroundObj) * (m_GroundLayerData.uNumObject + nCount));
    if (pObj == NULL)
        return;
        ...
        else if (pSelfObj->bOrder == pObj->bOrder)
        {
            ...
            else
            {
                while (nDest > 0)
                {
                    ...
                    if (pSelfObj->bOrder == pObj->bOrder && pSelfObj->nLayer == pObj->nLayer && (pSelfObj->nPositionY > pObj->nPositionY || (pSelfObj->nPositionY == pObj->nPositionY && pSelfObj->bRelateRegion > pObj->bRelateRegion)))
                    {
                        nDest--;
                    }
                    else
...
bool KScenePlaceRegionC::LoadAboveGroundObjects(KPakFile *pDataFile, unsigned int uSize)
{
    ...
    if (uSize < m_BiosData.Numbers.nNumBios * sizeof(KBuildinObj) + m_BiosData.Numbers.nNumBulidinLight * sizeof(KBuildInLightInfo) + sizeof(KBuildinObjFileHead))
    {
    ...
    if (m_BiosData.Numbers.nNumBios)
    {
        ...
        for (unsigned int i = m_BiosData.Numbers.nNumBios - m_BiosData.Numbers.nNumBiosAbove; i < m_BiosData.Numbers.nNumBios; i++)
        {
...
void KScenePlaceRegionC::PaintGroundDirect()
{
    ...
    for (nIndex = 0; nIndex < m_GroundLayerData.uNumObject; nIndex++, pObj++)
    {
        ...
        if (pObj->bRelateRegion == OBJ_IS_SELF_OWNED && pObj->nPositionX >= m_LeftTopCornerScenePos.x && pObj->nPositionY >= m_LeftTopCornerScenePos.y && pObj->nPositionX + pObj->nWidth < RegionRBPos.x && pObj->nPositionY + pObj->nHeight * 2 < RegionRBPos.y)
        {
            pGi->bRenderStyle = IMAGE_RENDER_STYLE_3LEVEL;
            pGi->nType = ISI_T_SPR;
            pGi->bRenderFlag = RUIMAGE_RENDER_FLAG_FRAME_DRAW;
    ...
    for (i = 0; i < 16; i++, nX += RWP_OBSTACLE_WIDTH)
    {
        ...
        for (j = 0; j < 32; j++, nY += RWP_OBSTACLE_HEIGHT)
        {
            ...
            if (nType == Obstacle_Empty)
                continue;
            Line[2].oEndPos.nX = Line[1].oEndPos.nX = Line[0].oEndPos.nX = Line[2].oPosition.nX = Line[1].oPosition.nX = Line[0].oPosition.nX = nX;
            Line[2].oEndPos.nY = Line[1].oEndPos.nY = Line[0].oEndPos.nY = Line[2].oPosition.nY = Line[1].oPosition.nY = Line[0].oPosition.nY = nY;
            nNumLine = 0;
            switch (nType)
            {
            case Obstacle_Full:
...
void KScenePlaceRegionC::PaintAboveHeadObj(KBuildinObj *pObj, RECT *pRepresentArea)
{
    ...
    if (pObj->nAniSpeed == 0)
    {
        ...
        if (SM_IsOutsideRepresentArea(pRepresentArea, pObj->ImgPos1.x, pObj->ImgPos1.y, pObj->ImgPos1.z, pObj->ImgPos2.x, pObj->ImgPos2.y, pObj->ImgPos2.z) == false)
        {
...
void KScenePlaceRegionC::GetBuildinObjs(KIpotBuildinObj *&pObjsPointList, unsigned int &nNumObjsPoint, KIpotBuildinObj *&pObjsLineList, unsigned int &nNumObjsLine, KIpotBuildinObj *&pObjsTreeList, unsigned int &nNumObjsTree)
{
    pObjsPointList = NULL;
    pObjsLineList = NULL;
    pObjsTreeList = NULL;
...
//## 获取区域内内建未初始化的对象的列表。
void KScenePlaceRegionC::GetBIOSBuildinObjs(KBuildinObj *&pObjsList, unsigned int &nNumObjs)
{
    pObjsList = NULL;
...
void KScenePlaceRegionC::EnterProcessArea(KRUImage *pImage)
{
    ...
    unsigned int i;
    unsigned int nCount = m_BiosData.Numbers.nNumBios - m_BiosData.Numbers.nNumBiosAbove;
    KIpotBuildinObj *pLeaf = NULL;
    KBuildinObj *pObj = NULL;

    if (m_BiosData.pLeafs == NULL && m_BiosData.pBios && m_BiosData.Numbers.nNumBios > m_BiosData.Numbers.nNumBiosAbove)
    {
        m_BiosData.pLeafs = (KIpotBuildinObj *)malloc(sizeof(KIpotBuildinObj) * nCount);
        if (m_BiosData.pLeafs)
        {
