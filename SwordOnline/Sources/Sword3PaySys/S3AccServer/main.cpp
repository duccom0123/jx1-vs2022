...
#include <initguid.h>
#define _WIN32_DCOM
// Thay thế heaven.dll interface bằng yasio
#include "../../MultiServer/Common/NetworkAdapter.h"
...
// Comment out old heaven.dll GUID definitions - not needed for yasio
...
// Comment out old heaven.dll function pointer - not needed for yasio
// typedef HRESULT(__stdcall *pfnCreateServerInterface)(REFIID riid, void **ppv);

void ServerEventNotify(LPVOID lpParam, const unsigned long &ulnID, const unsigned long &ulnEventType)
{
...
{
    KGLOG_PARAM LogParam = {"logs", "paysys", "", (KGLOG_OPTIONS)(LOG_OPTION_FILE | LOG_OPTION_CONSOLE), 10000 * 10};
    ...
    DWORD dwResult = KPIGetPrivateProfileString("Local", "port", szPort, szPort, 31, szINIPath);
    short siPort = atoi(szPort);
    dwResult = KPIGetPrivateProfileString("Local", "maxconnect", "10", szPort, 31, szINIPath);
    int snMaxPlayerCount = atoi(szPort);
    dwResult = KPIGetPrivateProfileString("Local", "precision", "1", szPort, 31, szINIPath);
    int snPrecision = atoi(szPort);
    dwResult = KPIGetPrivateProfileString("Local", "maxbuffer", "15", szPort, 31, szINIPath);
    int snMaxBuffer = atoi(szPort);
    dwResult = KPIGetPrivateProfileString("Local", "sizebuffer", "1048576", szPort, 31, szINIPath);
    int snBufferSize = atoi(szPort);

    dwResult = KPIGetPrivateProfileString("Local", "timelog", "3000", szPort, 31, szINIPath);
    int nTimelog = atoi(szPort);

    // Thay thế heaven.dll bằng yasio server (theo pattern Goddess.cpp)
    std::unique_ptr<CGameServer> pYasioServer = nullptr;

    // Tạo yasio game server với các tham số phù hợp (theo Goddess pattern)
    KGLogPrintf(LOG_INFO, "[Paysys] Creating yasio server with maxBuffer=%d, bufferSize=%d", snMaxBuffer, snBufferSize);

    pYasioServer = CreateGameServer(snMaxBuffer, snPrecision, snBufferSize, snBufferSize);

    if (!pYasioServer)
    {
        KGLogPrintf(LOG_ERR, "[Paysys] Failed to create yasio server!");
        ::MessageBox(NULL, "Initialization failed! Cannot create yasio server", "Warning", MB_OK | MB_ICONSTOP);
        goto Exit0;
    }

    KGLogPrintf(LOG_INFO, "[Paysys] Yasio server created successfully");

    _tcscpy(szINIPath, szExePath);
    _tcscat(szINIPath, "DataBase.ini");

    dwResult = KPIGetPrivateProfileString("Local", "DBConnects", "5", szPort, 31, szINIPath);
    ...
    if (pYasioServer == nullptr)
    {
        ::MessageBox(NULL, "Yasio Server Error!", "error", MB_OK | MB_ICONERROR);
        goto Exit0;
    }
    ...
    pSocketPool->Start(pYasioServer.get(), snMaxPlayerCount);

    pYasioServer->Startup();

    pYasioServer->RegisterMsgFilter((LPVOID)pSocketPool, ServerEventNotify);
    pYasioServer->OpenService(INADDR_ANY, siPort);
...
Exit0:
    ...
    if (pYasioServer)
    {
        pYasioServer->CloseService();
        pYasioServer->Cleanup();
        pYasioServer->Release();
        pYasioServer.reset();   // Smart pointer tự động cleanup
    }

    // Không cần FreeLibrary vì không dùng heaven.dll nữa
    // if (hModule)
    //     ::FreeLibrary(hModule);
